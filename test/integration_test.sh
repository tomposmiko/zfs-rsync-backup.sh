...
test_finalization_failures_mark_backup_failed() {
    local failure_type
    local test_dir
    local stub_dir
    local config_dir
    ...
    for failure_type in pre-run post-run snapshot-collision; do
        test_dir=$(mktemp -d)
        ...
        if [ "$failure_type" == "pre-run" ]; then
            printf '#!/bin/bash\nexit 7\n' > "$vault_root/config/pre-run.sh"
        elif [ "$failure_type" == "post-run" ]; then
            printf '#!/bin/bash\nexit 8\n' > "$vault_root/config/post-run.sh"
        fi

        if [ "$failure_type" == "snapshot-collision" ]; then
            ZRB_COMMAND_PATH="$stub_dir:/usr/bin:/bin" ZFS_TEST_LOG="$zfs_log" ZFS_SNAPSHOT_EXISTS_STATUS=0 "$sbin_dir/zrb.sh" -g "$config_dir" -v Projects/SZB > "$test_dir/zrb.out" 2>&1
        else
            ZRB_COMMAND_PATH="$stub_dir:/usr/bin:/bin" ZFS_TEST_LOG="$zfs_log" "$sbin_dir/zrb.sh" -g "$config_dir" -v Projects/SZB > "$test_dir/zrb.out" 2>&1
        fi

        backup_status=$?

        assert_equal "1" "$backup_status" "$failure_type backup status" || test_status=1
        assert_file_exists "$vault_root/FAILED" || test_status=1
        assert_path_missing "$vault_root/FINISHED" || test_status=1
        assert_path_missing "$vault_root/RUNNING" || test_status=1
        assert_path_missing "$vault_root/log/lock" || test_status=1
        assert_file_empty "$zfs_log" || test_status=1
...
run_test test_finalization_failures_mark_backup_failed
